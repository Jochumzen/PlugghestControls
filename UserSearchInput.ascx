<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UserSearchInput.ascx.cs" Inherits="Plugghest.Modules.PlugghestControls.UserSearchInput" %>


<input id="UserSearch" class="autocomplete" action-url="http://dnndev.me/DesktopModules/SearchBoost/SearchService.ashx" resulturl="#">
    ...
    $(function () {
        ...
        $("#UserSearch").catcomplete({
            source: function (request, response) {
                ...
                $.ajax({
                    type: "post",
                    url: _url + '?portalId=0&inst=511&sb-page=1&sb-pagesize=10&sb-search=' + request.term + '&format=json',
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
                        response($.map(data.SearchResults, function (item) {
                            return {
                                text: ' ' + item.Title.replace('&lt;', '<').replace('&gt;', '>') + ' ',
                                label: item.SearchKey.replace('avtsb.DisplayName-', ''),
